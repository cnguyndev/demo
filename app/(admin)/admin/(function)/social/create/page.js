...
export default function Page() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (!res.ok) throw new Error("Không thể tải danh sách các loại mạng xã hội.");

                const json = await res.json();
                setAllSocialIcons(Array.isArray(json) ? json : []);
            } catch (e) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.message || "Tạo liên kết thất bại.");
            }

            alert('Tạo liên kết mới thành công!');
    ...
    return (
        <div className="bg-white p-6 md:p-8 shadow-sm rounded-lg border max-w-2xl mx-auto">
            <h1 className="text-2xl font-bold text-gray-900 mb-6">Thêm Liên kết Mạng xã hội</h1>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end gap-3 border-t pt-6">
                    ...
                    <button type="submit" disabled={state.submitting} className="px-4 py-2.5 bg-indigo-600 text-white rounded-lg disabled:opacity-60">
                        {state.submitting ? 'Đang tạo...' : 'Tạo liên kết'}
                    </button>
                </div>
            </form>
        </div>
