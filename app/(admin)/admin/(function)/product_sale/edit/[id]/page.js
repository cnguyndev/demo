...
export default function Page() {
        ...
        if (!saleId) return
            ; (async () => {
                try {
                    ...
                    let saleRes = await fetch(`${API_BASE}/product-sale/${saleId}`, { cache: 'no-store' })
                    if (!saleRes.ok) throw new Error(`Không lấy được dữ liệu khuyến mãi (HTTP ${saleRes.status})`)
                    ...
                    setForm({
                        ...
                        price_sale: String(s.price_sale ?? ''),
                        date_begin: formatForDB(s.date_begin),
                        date_end: formatForDB(s.date_end),
                        status: Number(s.status ?? 1),
                    })

                    const allRes = await fetch(`${API_BASE}/product-sale`, { cache: 'no-store' })
                    if (allRes.ok) {
    ...
    useEffect(() => {
        ...
        ; (async () => {
            try {
                let detailRes = await fetch(`${API_BASE}/getAdminProduct/${id}`, { cache: 'no-store' })
                if (!detailRes.ok) throw new Error()
                const detail = await detailRes.json()
                setSelectedProductDetail(detail?.data || detail || null)
            } catch {
                setSelectedProductDetail(null)
            }
        })()
    ...
    const handlePickProduct = (p) => {
        setForm(prev => ({
            ...
            name: prev.name ? prev.name : `Khuyến mãi cho ${p.name || p.product_name || ''}`.trim(),
        }))
    }
