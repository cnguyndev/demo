...
/** Suy luận tổng/đã trả/chưa trả theo nhiều naming có thể có từ backend */
...
export default function PaymentResultPage() {
    ...
    const resultCodeParam = params.get('resultCode'); // có thể có từ cổng thanh toán
    const [order, setOrder] = useState(null);
    const [phase, setPhase] = useState('loading'); // loading | pending | success | fail | notfound | error
    const [errorMsg, setErrorMsg] = useState('');
    const [attempts, setAttempts] = useState(0);

    // dùng ref để đảm bảo chỉ clear giỏ 1 lần
    const clearedRef = useRef(false);
    // giữ timer để clear khi unmount
    const timerRef = useRef(null);
    // chặn setState sau unmount
    ...
    // Quyết định phase theo trạng thái đơn (tuỳ backend: 1=paid, 0=pending, khác=fail/pending)
    const decidePhase = (o) => {
        if (!o) return;
        // Nếu backend của bạn có enum status cụ thể, chỉnh lại nhánh bên dưới cho khớp
        if (o.status === 1 || o.paymentStatus === 'PAID' || o.isPaid === true) {
            setPhase('success');
        } else if (
            o.status === 0 ||
            ...
            // mặc định coi là pending để tiếp tục poll
            setPhase('pending');
        }
    };

    // Poll trạng thái đơn trong một khoảng giới hạn
    const startPolling = async (id) => {
        ...
        // Nếu đã success thì không cần poll
        if (first.status === 1 || first.paymentStatus === 'PAID' || first.isPaid) {
            return;
        }
        ...
        const tick = async () => {
            ...
            if (latest.status === 1 || latest.paymentStatus === 'PAID' || latest.isPaid) {
                setPhase('success');
                return;
            }
            ...
            tries += 1;
            if (tries >= maxTries) {
                // hết số lần thử: nếu vẫn chưa paid thì fail
                setPhase(
                    latest.status === 1 || latest.paymentStatus === 'PAID' || latest.isPaid
                        ? 'success'
                        : 'fail'
                );
                return;
    ...
    // Làm sạch URL: chỉ để lại ?orderId=... (loại resultCode/extra params)
    useEffect(() => {
        if (!orderId) return;
    ...
    const LoadingView = () => (
        <div className="text-center">
            ...
            <p className="text-gray-600">Đang kiểm tra trạng thái đơn hàng #{orderId}.</p>
        </div>
    );
        ...
        <div className="min-h-[70vh] flex items-center justify-center px-4 py-12 pt-40">
            <div className="max-w-2xl w-full bg-white rounded-3xl shadow-2xl p-10 border border-gray-100">
                <div className="flex items-center justify-center gap-3 mb-6">
                    ...
                    <h2 className="text-xl font-bold text-gray-900">Kết quả thanh toán</h2>
                </div>
